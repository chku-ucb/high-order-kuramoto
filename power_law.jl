...
function one_part(id, rK3, γ)
    Random.seed!(1234)
    println("Threads: ", Threads.nthreads())
    n = 2000
    mean_deg = 50
    upbd = n-1
    f(k_min) = k_min -  mean_deg*(γ-2)/(γ-1)*((upbd/k_min)^(1-γ)-1)/((upbd/k_min)^(2-γ)-1)
    lwbd = round(Int, find_zeros(f,1,n)[1])

    A2, degree2, target = create_L_CL_PL_bound(n, γ, lwbd, upbd)
    ...
    key_params = Dict("K2Crit" => K2Crit, "K3Crit" => K3Crit, "n" => n, "K2Crit_mean" => K2Crit_mean,
                      ...
                      "gamma" => γ, "rK3" => rK3)
    @save joinpath(script_dir,"results/res_pw_g_$(γ)_n_$(n)_ID_$(id)_rK3_$(rK3).jld2") k2rf R_tatf R_fdaf Rf_num k2rb R_tatb R_fdab Rb_num key_params
...
function main1()
    ...
    for γ in γs
        for rK3 in rK3s
            println("Running for γ = ", γ, " and rK3 = ", rK3)
            one_part(id, rK3, γ)
        end
    end
end
